// components/pages/blog/BlogListing.tsx
"use client";
...
export default function BlogListing({ initialBlogs }: BlogListingProps) {
  ...
  const allCategories = Array.from(new Set(initialBlogs.map(blog => blog.category)));
  const allTags = Array.from(new Set(initialBlogs.flatMap(blog => blog.tags)));
  
  // Filter blogs based on search, category, and tag filters
